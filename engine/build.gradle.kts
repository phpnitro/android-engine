...
android {
    ...
    defaultConfig {
        ...
        // defaultConfig DSL entirely ("Unresolved reference") — it was
        // always an application-only concept (a consuming app's own
        // targetSdk is what Play Store checks), vestigial on a library
        // either way.
    }
...
// JitPack's own recipe for an Android library (jitpack.io/docs/ANDROID/):
// a `release` MavenPublication under groupId "com.github.<owner>" — JitPack
// substitutes the real version in at build time (whatever git tag/commit was
// requested), this "unspecified" placeholder is only what a purely local
// `./gradlew publishToMavenLocal` would use.
afterEvaluate {
    publishing {
        publications {
            create<MavenPublication>("release") {
                from(components["release"])
                groupId = "com.github.phpnitro"
                artifactId = "android-engine"
            }
        }
    }
}
